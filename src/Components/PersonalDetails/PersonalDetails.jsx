...
export const PersonalDetails =()=>{
  ...
  const renderComponents =(details)=>{
      return <Row>
                {
                  !isEmpty(details.attributes) && details.attributes.map((item)=>{
                    if(get(item,"attribute.keyName") && get(item,"attribute.keyName")!== "template" && get(item,"attribute.keyName")!== "userId"){
                        const metaData= get(item,"metaData", []);
                    console.log("metaData",metaData);
                    const typeObj =metaData.find((i)=>i.keyName==="type") ||{};
                        return <React.Fragment key={get(item,"attribute.keyName")}>
                                  <Col span={4}>
                                    {get(item,"attribute.keyName")}
                                  </Col>
                                  <Col span={12}>
                                    {getRenderableComponentByType({type:typeObj.keyValue,value:get(item,"attribute.keyValue"),options:["Pune","Mumbai"]})}
                                  </Col>
                                  <Divider />
                              </React.Fragment>
                    }
